...
import { bg, centeredText, hide, jump, narrate, say, scene, setFlag, show } from "@/scenes/scriptTypes";
...
export const day5NadiaScene: VisualNovelCommand[] = [
  bg(
    bedroomAfternoonUrl,
    tx({
  ...
  say("nadia", "blush", tx({
    id: "Kalau kamu mau pergi, aku nggak bakalan nahan. Tapi kalau kamu tinggal… jangan cuma diem aja.",
    en: "If you want to leave, I won't stop you. But if you stay... don't just sit there in silence.",
    ja: "帰りたいなら止めない。でも残ってくれるなら…ただ黙ってないで。",
    ko: "가고 싶으면 붙잡지 않을게. 하지만 남을 거라면… 그냥 가만히 있지는 마.",
  })),
  narrate(tx({
  ...
  say("nadia", "blush", tx({
    id: "(suara pelan) Kamu hangat.",
    en: "(softly) You're warm.",
    ja: "（小さな声で）あったかい。",
    ko: "(작은 목소리로) 따뜻하네.",
  })),
  narrate(tx({
  ...
  narrate(tx({
    id: "Ciuman itu tidak terburu-buru. Nadia melakukannya pelan, seolah sedang memastikan Arka tidak menolak. Ketika Arka membalas, Nadia sedikit memperdalam ciumannya sebelum akhirnya melepaskan.",
    en: "The kiss is unhurried. Nadia takes it slowly, as if making sure Arka is not pulling away. When he kisses her back, she deepens it slightly before finally letting go.",
    ja: "そのキスは急いだものではなかった。アルカが拒まないことを確かめるように、ナディアはゆっくりと唇を重ねる。アルカが応えると、彼女は少しだけキスを深めてから、ようやく離れた。",
    ko: "키스는 서두르지 않았다. 나디아는 아르카가 거부하지 않는지 확인하듯 천천히 입을 맞췄다. 아르카가 화답하자 그녀는 입맞춤을 조금 더 깊게 한 뒤 마침내 떨어졌다.",
  })),
  narrate(tx({
    id: "Wajahnya masih dekat dengan Arka, napasnya sedikit lebih cepat.",
    en: "Her face remains close to Arka's, her breathing a little quicker.",
    ja: "彼女の顔はまだアルカのすぐ近くにあり、呼吸は少し速くなっていた。",
    ko: "그녀의 얼굴은 여전히 아르카와 가까웠고 숨은 조금 빨라져 있었다.",
  })),
  say("nadia", "blush", tx({
    id: "(berbisik) Makasih… sudah mau tinggal malam ini.",
    en: "(whispering) Thank you... for staying tonight.",
    ja: "（囁いて）ありがとう…今夜、残ってくれて。",
    ko: "(속삭이며) 고마워… 오늘 밤 있어 줘서.",
  })),
  narrate(tx({
    id: "Ia kembali menyandarkan kepala di bahu Arka, kali ini lebih rileks.",
    en: "She rests her head on Arka's shoulder again, more relaxed this time.",
    ja: "彼女は再びアルカの肩に頭を預けた。今度はさっきよりも力が抜けていた。",
    ko: "그녀는 다시 아르카의 어깨에 머리를 기댔다. 이번에는 훨씬 편안해 보였다.",
  })),
  say("nadia", "neutral", tx({
    id: "Besok aku mau tanya sesuatu yang penting. Soal arah stream aku ke depan.",
    en: "Tomorrow I want to ask you something important. About where I should take my streams from here.",
    ja: "明日、大事なことを聞きたい。これから配信をどうしていくかについて。",
    ko: "내일 중요한 걸 물어보고 싶어. 앞으로 방송을 어떤 방향으로 해 나갈지에 대해서.",
  })),
  say("arka", "neutral", tx({
    id: "Apa itu?",
    en: "What is it?",
  ...
  say("nadia", "smile", tx({
    id: "Nanti aja. Malam ini aku nggak mau mikir.",
    en: "Later. I don't want to think tonight.",
    ja: "また今度。今夜は考えたくない。",
    ko: "나중에. 오늘 밤은 생각하고 싶지 않아.",
  })),
  narrate(tx({
  ...
  say("nadia", "blush", tx({
    id: "Biarin aku begini dulu, ya.",
    en: "Let me stay like this for a while, okay?",
    ja: "しばらくこのままでいさせて。ね？",
    ko: "잠깐만 이대로 있게 해 줘. 응?",
  })),
  narrate(tx({
